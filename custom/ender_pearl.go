...
func (e *EnderPearl) Tick(w *world.World, current int64) {
	...
	e.pos, e.vel, e.yaw, e.pitch = m.Position(), m.Velocity().Mul(1.04), yaw, pitch
	e.mu.Unlock()
...
// EnderPearl is a smooth, greenish-blue item used to teleport and to make an eye of ender.
type EnderPearItem struct{}

// Use ...
func (e EnderPearItem) Use(w *world.World, user item.User, ctx *item.UseContext) bool {
	yaw, pitch := user.Rotation()
	pearl := EnderPearl{}

	entity := pearl.New(entity.EyePosition(user), entity.DirectionVector(user).Mul(1.5), yaw, pitch)
...
// Cooldown ...
func (EnderPearItem) Cooldown() time.Duration {
	return time.Second
}

// MaxCount ...
func (EnderPearItem) MaxCount() int {
	return 16
}

// EncodeItem ...
func (EnderPearItem) EncodeItem() (name string, meta int16) {
